        ...
        if train:
            ...
            if full_sort:
                ...
                for i, batch in rec_data_iter:
                    ...
                    # argpartition
                    # The negative sign "-" indicates a higher value
                    ind = np.argpartition(rating_pred, -20)[:, -20:]
                    # The subtable topk of each row is obtained by taking the corresponding value from the corresponding dimension according to the returned subscript
                    arr_ind = rating_pred[np.arange(len(rating_pred))[:, None], ind]
                    # The subtable is sorted in order from largest to smallest
                    arr_ind_argsort = np.argsort(arr_ind)[np.arange(len(rating_pred)), ::-1]
                    # Get the original subscript from ind again
