...
from recbole.utils import ensure_dir, get_local_time, early_stopping, calculate_valid_score, dict2str, \
    DataLoaderType, KGDataLoaderState, get_gpu_usage
...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        self.saved_model_file = os.path.join(config['log_dir'], saved_model_file)
        ...
        self.running_flag = config['running_flag'] # tools
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            # read running_param
            try:
                filePath = "./zone/running_param.txt"
                if os.path.exists(filePath):
                    fileObj = open(filePath, 'r')
                    text = fileObj.read()
                    fileObj.close()
                    if text.__contains__(self.running_flag):
                        break
        ...
        if self.config['open_ali_uni']:
            ...
            data.to_csv(self.config['log_dir'] + '/' + self.config['model'] + '-' + self.config['dataset'] + '-' + self.config['contrast'] + '.csv', index=False)
...
class SimDCLTrainer(Trainer):
    r"""SimDCLTrainer is designed for SimDCL, which is a sequence recommendation method.

    """

    def __init__(self, config, model):
        super(SimDCLTrainer, self).__init__(config, model)
