...
def init_logger(config):
    ...
    LOGROOT += config['model'] + '/{}/{}-bs{}-lmd{}-sem{}-{}-lr{}-l2{}-tau{}-{}-DPh{}-DPa{}/'.format(
        config['dataset'], get_local_time(), config['train_batch_size'], config['lmd'], config['lmd_sem'], config['contrast'],
        config['learning_rate'], config['weight_decay'], config['tau'], config['sim'],
